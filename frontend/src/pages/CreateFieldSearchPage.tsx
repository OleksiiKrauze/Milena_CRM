...
export function CreateFieldSearchPage() {
  ...
  const handleGenerateGrid = async () => {
    ...
    try {
      // Note: This will need to be called after the field search is created
      // For now, we'll just show a message
      setApiError('Спочатку збережіть виїзд, потім зможете згенерувати сітку');
    } catch (error: any) {
      setApiError(error.message || 'Помилка генерації сітки');
    } finally {
  ...
  return (
    <div className="min-h-screen pb-nav">
      ...
      <Container className="py-6">
          ...
          {searchData && (
            <Card>
              ...
              <CardContent>
                ...
                {searchData.latest_orientation_image && (
                  <div className="mt-4">
                    <p className="text-sm text-gray-600 mb-2">Ориентировка</p>
                    <img
                      src={searchData.latest_orientation_image}
                      alt="Ориентировка"
        ...
        {isOrientationFullscreen && searchData?.latest_orientation_image && (
          <div
